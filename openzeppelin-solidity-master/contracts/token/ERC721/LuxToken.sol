...
contract LuxarityToken is ERC721Full, Ownable {
  ...
  //state variables
    ...
    //token mapping
    mapping (bytes32 => LuxToken) luxTokens;
  ...
  //events
    ...
    event MadeToCharityToLuxarity (bytes32 _donorHash, uint256 _tokenId);
    ...
    event MadeToCharityToCharity (uint256 _amountDonated, string _charityName, bytes32 _proofHash, string _donationProof, bytes32 _donationMadeHash);
  ...
  //cuase functions
    ...
    function mintItem(string _tokenURI, string _brand, string _size, string _gender, string _style, _donorHash) public onlyOwner returns (uint) {
      ...
      bytes32 tokenHash = keccak256(_brand, _size, _style, _gender, _tokenURI, msg.sender, msg.sender, _donorHash);
      //check if token doesn't exist yet
      require(luxTokens[tokenHash].generation == address(0));
      //increment tokenIndex
      tokenIndex += 1;
      //create token struct
      luxTokens[tokenHash] = LuxToken(tokenIndex, _brand, _size, _style, _gender, _tokenURI, msg.sender, msg.sender, _donorHash);
      //mint token
      _mint(msg.sender, tokenIndex);
      //set metadata
      _setTokenURI(tokenIndex, _tokenURI);
      //emit event
      emit MintedToken(_tokenId, msg.sender, _tokenURI);
      //return
      return tokenIndex;
    }
    ...
    function soldItem(bytes32 _buyerID, uint256 _itemCost, bytes32 _redemptionHash, uint256 _tokenId) public onlyOwner returns (bool) {
      ...
      if (!buyers[_buyerID].exists) {
        ...
        buyers[_buyerID] = Buyer(_itemCost, true, address(0));
      } else {
      ...
      if (!soldTokens[_tokenId].sold) {
        ...
        emit SoldToken(_tokenId, buyers[_buyerID].buyerHash, soldTokens[_tokenId].cost);
        //return
        return true;
    ...
    function chooseDonation(bytes32 _buyerID, string _charityName, uint256 _chosenDonateAmount) public onlyBuyer(_buyerID) returns (bool) {
      ...
      if (leftover <= _chosenDonateAmount) {
        ...
        chosenDonations[totalChosenDonations] = ChosenDonation(charityName, _chosenDonateAmount, _buyerID);
    ...
    function makeDonation(bytes32 _proofHash, string _proof, uint256 _madeDonationAmount, string _charityName) public onlyOwner returns (bool) {
      ...
      bytes32 donationMadeHash = keccak256(_charityName, _madeDonationAmount, _proofHash, msg.sender, _proof);
      ...
      //emit event
      emit DonationMadeToCharity(uint256 _madeDonationAmount, _charityName, _proofHash, _proof, donationMadeHash);
      //return
      return true;
    }
    ...
    function redeemItem(bytes32 _buyerID, bytes32 _redemptionHash, address _buyerAddress, uint256 _tokenId, address _fromLuxarity) public onlyBuyer(_buyerID) returns (bool) {
      ...
      transferFrom(_fromLuxarity, _buyerAddress, _tokenId);
      //emit event
    ...
    //safe redeem token
      ...
      //(3) updates SaleDetails address for token id indicating new owne
      //(4) emits RedeemedToken event
    function safeRedeemItem(bytes32 _buyerID, bytes32 _redemptionHash, address _buyerAddress) public onlyBuyer(_buyerID) returns (bool) {
      //make sure address is valid
      require(_buyerAddress != address(0));
      //make sure that sold item exists to be redeemed
      require(soldTokens[_tokenId].buyerHash == _buyerID);
      ...
      luxTokens[_tokenId].owner = _buyerAddress;
      //conduct transfer from Luxarity to address
      safeTransferFrom(_from, _buyerAddress, _tokenId);
      //updates salesdetails struct
      soldTokens[_tokenId].redeemed = true;
      //emit event
      emit RedeemedToken(_tokenId, _buyerID, _buyerAddress);
  ...
  //call functions
    ...
    function getRedemption(uint _tokenId) public view returns (uint) {
      return soldTokens[_tokenId].redeemed;
		}
    //get boolean to see if NFT has been sold or not
    function getTokenSold(uint _tokenId) public view returns (uint) {
    ...
    function getDonationAmount(uint _donorId) public view returns (uint) {
